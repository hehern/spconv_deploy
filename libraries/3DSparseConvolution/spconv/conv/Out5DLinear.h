...
struct Out5DLinear {
  __forceinline__ __host__ __device__ static std::array<int, 2> initial_offset(int thread_idx)   {
    ...
    int group_offset = group_idx * 8 * 4;
    // 0
  ...
  __forceinline__ __host__ __device__ static std::array<int64_t, 3> iteration_inc_params(int stride)   {
    std::array<int64_t, 3> increments{};
  ...
  __forceinline__ __host__ __device__ static std::array<int64_t, 4> iteration_advance_params(int stride)   {
    std::array<int64_t, 4> advances{};
    // so advances[0] == 
    advances[0] =
        stride * 1 * 1 * 2 * 8;
