#pragma once
// 从 cumm 生成代码移植: out_iter/OutIterator.h
#include <cuda_fp16.h>
...
using OutThreadMap = Out5DLinear;
...
struct OutIterator {
  ...
  int counts_[3];
  int64_t indices_[1];
  __forceinline__ __device__  OutIterator(OutIteratorParams const& params, half * ptr, std::array<int, 2> extent, std::array<int, 2> offset_2d, int thread_idx) : params_(params)  {
    counts_[0] = 0;
    counts_[1] = 0;
    counts_[2] = 0;
    auto tmap_offset = OutThreadMap::initial_offset(thread_idx);
  ...
  __forceinline__ __device__ void store_with_offset(std::array<half, 16> const & frag, int32_t offset)   {
    ...
    aligned_array<int4, 1, 16> const *frag_ptr = reinterpret_cast<aligned_array<int4, 1, 16> const *>(&frag);
    #pragma unroll
    for (int cluster = 0; cluster < 1; ++cluster){
      #pragma unroll
      for (int group = 0; group < 1; ++group){
        ...
        for (int row = 0; row < 1; ++row){
          ...
          aligned_array<int4, 1, 16>  *memory_pointer =
              reinterpret_cast<aligned_array<int4, 1, 16>  *>(cur_pointer + offset + indices_[frag_row_idx]);
          #pragma unroll
          for (int column = 0; column < 2; ++column){
            bool guard = row_guard && column_masks_[column][0];
            global_store16(&frag_ptr[frag_row_idx *  2 + column],
